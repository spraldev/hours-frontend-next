...
export async function exportStudentsHoursToCSVByClass(students: Student[], hours: Hour[], graduationYear: number) {
  if (students.length === 0) {
    throw new Error("No students data available to export")
  }

  // Filter students by graduating year
  const classStudents = students.filter(student => student.graduatingYear === graduationYear)
  
  if (classStudents.length === 0) {
    throw new Error(`No students found for Class of ${graduationYear}`)
  }

  // Create a map of student hours for quick lookup
  const studentHoursMap = new Map<string, number>()
  
  // Calculate total approved hours for each student in the class
  hours.forEach(hour => {
    if (hour.status === 'approved') {
      const studentId = typeof hour.student === 'string' ? hour.student : hour.student?._id
      if (studentId && hour.hours) {
        // Only include hours for students in the selected class
        const isStudentInClass = classStudents.some(student => student._id === studentId)
        if (isStudentInClass) {
          const currentHours = studentHoursMap.get(studentId) || 0
          studentHoursMap.set(studentId, currentHours + hour.hours)
        }
  ...
  // CSV Headers
  const headers = ["Student Name", "Student ID", "Email", "Graduating Year", "Total Approved Hours"]
  ...
  // Sort by total hours (descending) then by name
  rows.sort((a, b) => {
    const hoursA = parseFloat(a[4] as string)
    const hoursB = parseFloat(b[4] as string)
    if (hoursA !== hoursB) {
      return hoursB - hoursA // Descending by hours
    }
    return (a[0] as string).localeCompare(b[0] as string) // Then by name
  ...
  link.setAttribute("href", url)
  link.setAttribute("download", `class-of-${graduationYear}-hours-${new Date().toISOString().split('T')[0]}.csv`)
  link.style.visibility = "hidden"
