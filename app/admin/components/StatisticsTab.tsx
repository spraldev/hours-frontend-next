...
export function StatisticsTab({ students, supervisors, hours, organizations }: StatisticsTabProps) {
  ...
  const approvedSupervisors = supervisors.filter((s) => s.isApproved).length
  ...
  const pendingHours = hours.filter((h) => h.status === 'pending').reduce((sum, h) => sum + h.hours, 0)
  const approvalRate = hours.length > 0 ? Math.round((hours.filter((h) => h.status === 'approved').length / hours.length) * 100) : 0
  const avgHoursPerStudent = students.length > 0 ? Math.round((approvedHours / students.length) * 10) / 10 : 0
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {students.length > 0 ? Math.round((activeStudents / students.length) * 100) : 0}% of total
              </div>
            </div>
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                {supervisors.length > 0 ? Math.round((approvedSupervisors / supervisors.length) * 100) : 0}% of total
              </div>
            </div>
            <div className="text-center">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">{hours.length} entries submitted</div>
            </div>
            <div className="text-center">
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">{hours.filter((h) => h.status === 'pending').length} entries</div>
            </div>
            <div className="text-center">
